...
enum Puzzle4_2: Puzzle {
	...
	class Guard: Equatable, CustomStringConvertible {
		...
		var maxMinute: (Int, Int) {
			...
			let max = counts.max(by: { $0.1 < $1.1 }) ?? (0, 0)
			
			return max
		}
		
		var identifier: Int {
			return id * maxMinute.0
		}
	...
	class Guards: CustomStringConvertible {
		...
		var bestChoice: Int {
			...
			let best = guards.filter({ $0.events.contains(where: { e in e.event == .sleep }) }).max(by: { $0.maxMinute.1 < $1.maxMinute.1 })!
			print(best.id)
			print(best)
